 ...
 * 
 */
package com.neu.mr.inhomeparser;
...
public class Bz2Parser {
	...
	private static final Pattern linkPattern = Pattern.compile("^\\..*/([^~]+)\\.html$");
	...
	public static String parseLine(String line) {
		...
		try {
			...
			if (matcher.find()) {
				...
				StringBuilder stringBuilder = new StringBuilder(pageName + AppConstants.TILDE);
				if(linkPageNames.isEmpty()){
					stringBuilder.append(AppConstants.TILDE);
				}else{
					for(String outlink : linkPageNames){
						stringBuilder.append(AppConstants.TILDE + outlink);
					}
				}
				for(String outlink : linkPageNames){
					stringBuilder.append(AppConstants.NEW_LINE);
					stringBuilder.append(outlink + AppConstants.TILDE + AppConstants.TILDE);
				}
